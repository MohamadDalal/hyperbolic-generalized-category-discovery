...
# Default arguments: sbatch new_bash_scripts/contrastive_train-angle_SGD-HCD_DINOv2.sh cub 1.0 0.25 2.3 false 0

# Script arguments
container_path="${HOME}/pytorch25-06.sif"

# Dynamically set output and error filenames using job ID and iteration
outfile="logs/GCD-${data}-Hyperbolic-max_grad-${max_grad}-avg_grad-${avg_grad}-c-${c}-HypCD-${HypCD_mode}-Seed${seed}.out"

exp_id="GCD-${data}-Hyperbolic-max_grad-${max_grad}-avg_grad-${avg_grad}-c-${c}-HypCD-${HypCD_mode}-Seed${seed}"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id "${data}-Hyperbolic-max_grad-${max_grad}-avg_grad-${avg_grad}-c-${c}-HypCD-${HypCD_mode}-Seed${seed}-Train" \
            --hyperbolic 'True' \
            --kmeans 'True' \
            --kmeans_frequency 30 \
            --curvature '0.05' \

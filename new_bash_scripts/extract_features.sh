...
outfile="logs/GCD-Extract-${data}-Hyperbolic-max_grad-${max_grad}-avg_grad-${avg_grad}-c-${c}-HypCD-${HypCD_mode}-Seed${seed}-Euclidean${Euclidean}.out"
...
if [ "$Euclidean" = "True" ]; then
    ...
    --exp_id "_Hyperbolic-max_grad-${max_grad}-avg_grad-${avg_grad}-c-${c}-HypCD-${HypCD_mode}-Seed${seed}_Euclidean" --hyperbolic 'True' --poincare 'False' --euclidean_clipping 2.3 --remove_dyno_head 'True' --mlp_out_dim 256 --use_dinov2 'True' --HypCD_mode ${HypCD_mode}
else
    echo "Extracting features in Hyperbolic space"
    srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.clustering.extract_features --dataset ${data} --use_best_model 'False' \
    --warmup_model_dir "/home/create.aau.dk/lt61yz/P10-project/hyperbolic-generalized-category-discovery/osr_novel_categories/metric_learn_gcd/log/${data}-Hyperbolic-max_grad-${max_grad}-avg_grad-${avg_grad}-c-${c}-HypCD-${HypCD_mode}-Seed${seed}-Train/checkpoints/model_best_loss.pt" \
    --exp_id "_Hyperbolic-max_grad-${max_grad}-avg_grad-${avg_grad}-c-${c}-HypCD-${HypCD_mode}-Seed${seed}" --hyperbolic 'True' --poincare 'False' --euclidean_clipping 2.3 --remove_dyno_head 'False' --mlp_out_dim 256 --use_dinov2 'True' --HypCD_mode ${HypCD_mode}
fi

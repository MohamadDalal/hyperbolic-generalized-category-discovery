...
#SBATCH --output="logs/GCD-CIFAR10-Hyperbolic-Angle-SGD-HCD2-DINOv2.log"
#SBATCH --job-name="GCD-CIFAR10-Hyperbolic-Angle-SGD-HCD2-DINOv2"
...
outfile="logs/GCD-CIFAR10-Hyperbolic-Angle-SGD-HCD2-DINOv2.out"
...
if [ $restarts -gt 0 ]; then
    srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'CIFAR10-Hyperbolic-Angle-SGD-HCD2-DINOv2-Train' \
            --hyperbolic 'True' \
            --kmeans 'True' \
            ...
            --mlp_out_dim 256 \
            --use_dinov2 'True' \
            --checkpoint_path 'osr_novel_categories/metric_learn_gcd/log/CIFAR10-Hyperbolic-Angle-SGD-HCD2-DINOv2-Train/checkpoints/model.pt'
            #--use_adam 'True' \
else
    srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            --dataset_name 'cifar10' \

...
#SBATCH --output="logs/GCD-Extract-Cifar10-Hyperbolic.log"
#SBATCH --job-name="GCD-Extract-Cifar10-Hyperbolic"
...
outfile="logs/GCD-Extract-Cifar10-Hyperbolic.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.clustering.extract_features --dataset cifar10 --use_best_model 'True' \
 --warmup_model_dir '/ceph/home/student.aau.dk/mdalal20/P10-project/hyperbolic-generalized-category-discovery/osr_novel_categories/metric_learn_gcd/log/Cifar10-Hyperbolic-Train/checkpoints/model.pt' \

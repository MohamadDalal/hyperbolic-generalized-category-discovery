...
#SBATCH --output="logs/GCD-CUB-Hyperbolic-Angle-SGD.log"
#SBATCH --job-name="GCD-CUB-Hyperbolic-Angle-SGD"
...
outfile="logs/GCD-CUB-Hyperbolic-Angle-SGD.out"

exp_id="GCD-CUB-Hyperbolic-Angle-SGD"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'CUB-Hyperbolic-Angle-SGD-Train' \
            --hyperbolic 'True' \
            --kmeans 'True' \
            --kmeans_frequency 20 \
            --curvature '0.1' \

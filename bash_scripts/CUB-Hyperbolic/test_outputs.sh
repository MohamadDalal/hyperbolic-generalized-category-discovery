...
#SBATCH --output="logs/GCD-Test-CUB-Hyperbolic.log"
#SBATCH --job-name="GCD-Test-CUB-Hyperbolic"
...
outfile="logs/GCD-Test-CUB-Hyperbolic.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.test_outputs.test_outputs --dataset_name cub --warmup_model_exp_id 'Hyperbolic_best' --hyperbolic 'True'

...
#SBATCH --output="logs/GCD-CUB-Hyperbolic-Angle.log"
#SBATCH --job-name="GCD-CUB-Hyperbolic-Angle"
...
outfile="logs/GCD-CUB-Hyperbolic-Angle.out"

exp_id="GCD-CUB-Hyperbolic-Angle"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'CUB-Hyperbolic-Angle-Train' \
            --hyperbolic 'True' \
            --kmeans 'True' \
            --kmeans_frequency 20 \
            --curvature '0.05' \

...
#SBATCH --output="logs/GCD-CUB-Hyperbolic-Poincare.log"
#SBATCH --job-name="GCD-CUB-Hyperbolic-Poincare"
...
outfile="logs/GCD-CUB-Hyperbolic-Poincare.out"

exp_id="GCD-CUB-Hyperbolic-Poincare"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'CUB-Hyperbolic-Poincare-Train' \
            --hyperbolic 'True' \
            --kmeans 'True' \
            --kmeans_frequency 20 \
            --curvature '1' \

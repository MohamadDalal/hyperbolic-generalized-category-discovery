...
#SBATCH --output="logs/GCD-CUB-Hyperbolic-Angle-SGD-HCD-DINOv2.log"
#SBATCH --job-name="GCD-CUB-Hyperbolic-Angle-SGD-HCD-DINOv2"
...
outfile="logs/GCD-CUB-Hyperbolic-Angle-SGD-HCD-DINOv2.out"

exp_id="GCD-CUB-Hyperbolic-Angle-SGD-HCD-DINOv2"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'CUB-Hyperbolic-Angle-SGD-HCD-DINOv2-Train' \
            --hyperbolic 'True' \
            --kmeans 'True' \

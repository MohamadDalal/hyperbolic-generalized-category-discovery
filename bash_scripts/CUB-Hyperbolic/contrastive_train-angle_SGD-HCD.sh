...
#SBATCH --output="logs/GCD-CUB-Hyperbolic-Angle-SGD-HCD.log"
#SBATCH --job-name="GCD-CUB-Hyperbolic-Angle-SGD-HCD"
...
outfile="logs/GCD-CUB-Hyperbolic-Angle-SGD-HCD.out"

exp_id="GCD-CUB-Hyperbolic-Angle-SGD-HCD"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'CUB-Hyperbolic-Angle-SGD-HCD-Train' \
            --hyperbolic 'True' \
            --kmeans 'True' \

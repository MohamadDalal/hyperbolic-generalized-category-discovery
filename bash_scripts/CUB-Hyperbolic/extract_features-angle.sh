...
#SBATCH --output="logs/GCD-Extract-CUB-Hyperbolic-Angle.log"
#SBATCH --job-name="GCD-Extract-CUB-Hyperbolic-Angle"
#SBATCH --time=2:00:00
...
outfile="logs/GCD-Extract-CUB-Hyperbolic-Angle.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.clustering.extract_features --dataset cub --use_best_model 'False' \
 --warmup_model_dir '/ceph/home/student.aau.dk/mdalal20/P10-project/hyperbolic-generalized-category-discovery/osr_novel_categories/metric_learn_gcd/log/CUB-Hyperbolic-Angle-Train/checkpoints/model_best_loss.pt' \

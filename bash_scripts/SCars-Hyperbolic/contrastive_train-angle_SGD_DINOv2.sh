...
#SBATCH --output="logs/GCD-SCars-Hyperbolic-Angle-SGD4-DINOv2.log"
#SBATCH --job-name="GCD-SCars-Hyperbolic-Angle-SGD4-DINOv2"
...
outfile="logs/GCD-SCars-Hyperbolic-Angle-SGD4-DINOv2.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'SCars-Hyperbolic-Angle-SGD4-DINOv2-Train' \
            --hyperbolic 'True' \
            --kmeans 'True' \

...
#SBATCH --output="logs/GCD-Test-SCars-Hyperbolic-HCD.log"
#SBATCH --job-name="GCD-Test-SCars-Hyperbolic-HCD"
...
outfile="logs/GCD-Test-SCars-Hyperbolic-HCD.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.test_outputs.test_outputs --dataset_name scars --warmup_model_exp_id 'Hyperbolic-HCD' \

...
#SBATCH --output="logs/GCD-SCars-Hyperbolic-HCD-DINOv2.log"
#SBATCH --job-name="GCD-SCars-Hyperbolic-HCD-DINOv2"
...
outfile="logs/GCD-SCars-Hyperbolic-HCD-DINOv2.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'SCars-Hyperbolic-HCD-DINOv2-Train' \
            --hyperbolic 'True' \
            --poincare 'True' \
            --kmeans 'True' \
            --kmeans_frequency 20 \

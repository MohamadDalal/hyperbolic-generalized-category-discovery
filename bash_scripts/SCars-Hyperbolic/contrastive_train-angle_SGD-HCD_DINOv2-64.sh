...
#SBATCH --output="logs/GCD-SCars-Hyperbolic-Angle-SGD-HCD-DINOv2-64.log"
#SBATCH --job-name="GCD-SCars-Hyperbolic-Angle-SGD-HCD-DINOv2-64"
...
outfile="logs/GCD-SCars-Hyperbolic-Angle-SGD-HCD-DINOv2-64.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'SCars-Hyperbolic-Angle-SGD-HCD-DINOv2-64-Train' \
            --hyperbolic 'True' \
            --kmeans 'True' \

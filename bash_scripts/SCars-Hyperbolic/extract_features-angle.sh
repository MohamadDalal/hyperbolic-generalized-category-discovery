...
#SBATCH --output="logs/GCD-Extract-SCars-Hyperbolic-Angle.log"
#SBATCH --job-name="GCD-Extract-SCars-Hyperbolic-Angle"
#SBATCH --time=2:00:00
...
outfile="logs/GCD-Extract-SCars-Hyperbolic-Angle.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.clustering.extract_features --dataset scars --use_best_model 'False' \
 --warmup_model_dir '/ceph/home/student.aau.dk/mdalal20/P10-project/hyperbolic-generalized-category-discovery/osr_novel_categories/metric_learn_gcd/log/SCars-Hyperbolic-Angle-Train/checkpoints/model_best_loss.pt' \

...
#SBATCH --output="logs/GCD-Extract-SCars-Hyperbolic-HCD-angle-only.log"
#SBATCH --job-name="GCD-Extract-SCars-Hyperbolic-HCD-angle-only"
...
outfile="logs/GCD-Extract-SCars-Hyperbolic-HCD-angle-only.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.clustering.extract_features --dataset scars --use_best_model 'False' \
 --warmup_model_dir '/ceph/home/student.aau.dk/mdalal20/P10-project/hyperbolic-generalized-category-discovery/osr_novel_categories/metric_learn_gcd/log/SCars-Hyperbolic-HCD-angle-only-Train/checkpoints/model_best_loss.pt' \

...
#SBATCH --output="logs/GCD-SCars-Hyperbolic-HCD-angle-only-DINOv2.log"
#SBATCH --job-name="GCD-SCars-Hyperbolic-HCD-angle-only-DINOv2"
...
outfile="logs/GCD-SCars-Hyperbolic-HCD-angle-only-DINOv2.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'SCars-Hyperbolic-HCD-angle-only-DINOv2-Train' \
            --hyperbolic 'True' \
            --poincare 'True' \
            --kmeans 'True' \
            --kmeans_frequency 20 \
            ...
            --mlp_out_dim 256 \
            --use_dinov2 'True' \
            #--checkpoint_path '/ceph/home/student.aau.dk/mdalal20/P10-project/hyperbolic-generalized-category-discovery/osr_novel_categories/metric_learn_gcd/log/SCars-Hyperbolic-HCD-angle-only-DINOv2-Train/checkpoints/model.pt' \

...
#SBATCH --output="logs/GCD-KMeans-SCars-Hyperbolic-Angle-SGD-HCD.log"
#SBATCH --job-name="GCD-KMeans-SCars-Hyperbolic-Angle-SGD-HCD"
...
outfile="logs/GCD-KMeans-SCars-Hyperbolic-Angle-SGD-HCD.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.clustering.k_means --dataset 'scars' --semi_sup 'True' --use_ssb_splits 'True' \
 --use_best_model 'False' --max_kmeans_iter 200 --k_means_init 100 --warmup_model_exp_id 'Hyperbolic-Angle-SGD-HCD_Euclidean' --hyperbolic 'False' --poincare 'False' #--K 98

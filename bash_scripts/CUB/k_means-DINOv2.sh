...
#SBATCH --output="logs/GCD-KMeans-CUB-DINOv2.log"
#SBATCH --job-name="GCD-KMeans-CUB-DINOv2"
...
outfile="logs/GCD-KMeans-CUB-DINOv2.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.clustering.k_means --dataset 'cub' --semi_sup 'True' --use_ssb_splits 'True' \
 --use_best_model 'False' --max_kmeans_iter 200 --k_means_init 100 --warmup_model_exp_id 'Euclidean-DINOv2' --K 550

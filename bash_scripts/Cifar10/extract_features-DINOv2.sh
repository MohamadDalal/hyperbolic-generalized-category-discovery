...
#SBATCH --output="logs/GCD-Extract-CIFAR10-DINOv2.log"
#SBATCH --job-name="GCD-Extract-CIFAR10-DINOv2"
...
outfile="logs/GCD-Extract-CIFAR10-DINOv2.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.clustering.extract_features --dataset cifar10 --use_best_model 'False' \
 --warmup_model_dir '/ceph/home/student.aau.dk/mdalal20/P10-project/hyperbolic-generalized-category-discovery/osr_novel_categories/metric_learn_gcd/log/CIFAR10-DINOv2-Train/checkpoints/model_best_loss.pt' \

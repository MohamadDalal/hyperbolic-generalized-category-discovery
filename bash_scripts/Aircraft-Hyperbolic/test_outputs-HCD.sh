...
#SBATCH --output="logs/GCD-Test-Aircraft-Hyperbolic-HCD.log"
#SBATCH --job-name="GCD-Test-Aircraft-Hyperbolic-HCD"
...
outfile="logs/GCD-Test-Aircraft-Hyperbolic-HCD.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.test_outputs.test_outputs --dataset_name aircraft --warmup_model_exp_id 'Hyperbolic-HCD' \
 --hyperbolic 'True' --poincare 'True'

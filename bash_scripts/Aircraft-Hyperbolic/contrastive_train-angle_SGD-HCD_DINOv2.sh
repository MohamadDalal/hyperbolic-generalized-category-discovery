...
#SBATCH --output="logs/GCD-Aircraft-Hyperbolic-Angle-SGD-HCD-DINOv2.log"
#SBATCH --job-name="GCD-Aircraft-Hyperbolic-Angle-SGD-HCD-DINOv2"
...
outfile="logs/GCD-Aircraft-Hyperbolic-Angle-SGD-HCD-DINOv2.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'Aircraft-Hyperbolic-Angle-SGD-HCD-DINOv2-Train' \
            --hyperbolic 'True' \
            --kmeans 'True' \

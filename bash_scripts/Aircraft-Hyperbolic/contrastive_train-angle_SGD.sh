...
#SBATCH --output="logs/GCD-Aircraft-Hyperbolic-Angle-SGD.log"
#SBATCH --job-name="GCD-Aircraft-Hyperbolic-Angle-SGD"
...
outfile="logs/GCD-Aircraft-Hyperbolic-Angle-SGD.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'Aircraft-Hyperbolic-Angle-SGD-Train' \
            --hyperbolic 'True' \
            --kmeans 'True' \
            --kmeans_frequency 20 \
            --curvature 0.1 \

...
#SBATCH --output="logs/GCD-Aircraft-Hyperbolic-Angle-SGD6.log"
#SBATCH --job-name="GCD-Aircraft-Hyperbolic-Angle-SGD6"
...
outfile="logs/GCD-Aircraft-Hyperbolic-Angle-SGD6.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'Aircraft-Hyperbolic-Angle-SGD6-Train' \
            --hyperbolic 'True' \
            --kmeans 'True' \

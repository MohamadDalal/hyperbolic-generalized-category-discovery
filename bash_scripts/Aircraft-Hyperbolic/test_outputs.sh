...
#SBATCH --output="logs/GCD-Test-Aircraft-Hyperbolic.log"
#SBATCH --job-name="GCD-Test-Aircraft-Hyperbolic"
...
outfile="logs/GCD-Test-Aircraft-Hyperbolic.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.test_outputs.test_outputs --dataset_name aircraft --warmup_model_exp_id 'Hyperbolic_best' --hyperbolic 'True'

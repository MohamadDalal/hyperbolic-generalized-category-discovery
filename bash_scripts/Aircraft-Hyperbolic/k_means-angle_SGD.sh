...
#SBATCH --output="logs/GCD-KMeans-Aircraft-Hyperbolic-Angle-SGD4.log"
#SBATCH --job-name="GCD-KMeans-Aircraft-Hyperbolic-Angle-SGD4"
...
outfile="logs/GCD-KMeans-Aircraft-Hyperbolic-Angle-SGD4.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.clustering.k_means --dataset 'aircraft' --semi_sup 'True' --use_ssb_splits 'True' \
 --use_best_model 'False' --max_kmeans_iter 200 --k_means_init 100 --warmup_model_exp_id 'Hyperbolic-Angle-SGD4' --hyperbolic 'True' --poincare 'False' #--K 50

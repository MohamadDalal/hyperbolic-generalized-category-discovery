...
#SBATCH --output="logs/GCD-Aircraft-Hyperbolic-Poincare.log"
#SBATCH --job-name="GCD-Aircraft-Hyperbolic-Poincare"
...
outfile="logs/GCD-Aircraft-Hyperbolic-Poincare.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'Aircraft-Hyperbolic-Poincare-Train' \
            --hyperbolic 'True' \
            --kmeans 'True' \
            --kmeans_frequency 20 \
            --curvature '1.0' \

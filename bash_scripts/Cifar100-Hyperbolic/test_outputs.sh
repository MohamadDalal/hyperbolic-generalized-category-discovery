...
#SBATCH --output="logs/GCD-Test-Cifar100-Hyperbolic.log"
#SBATCH --job-name="GCD-Test-Cifar100-Hyperbolic"
...
outfile="logs/GCD-Test-Cifar100-Hyperbolic.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.test_outputs.test_outputs --dataset_name cifar100 --warmup_model_exp_id 'Hyperbolic_best' --hyperbolic 'True'

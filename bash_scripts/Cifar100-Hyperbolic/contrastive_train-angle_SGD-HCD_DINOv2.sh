...
#SBATCH --output="logs/GCD-CIFAR100-Hyperbolic-Angle-SGD-HCD-DINOv2.log"
#SBATCH --job-name="GCD-CIFAR100-Hyperbolic-Angle-SGD-HCD-DINOv2"
...
outfile="logs/GCD-CIFAR100-Hyperbolic-Angle-SGD-HCD-DINOv2.out"
...
if [ $restarts -gt -1 ]; then
    srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v2' \
            --exp_id 'CIFAR100-Hyperbolic-Angle-SGD-HCD-DINOv2-Train' \
            --hyperbolic 'True' \
            --kmeans 'True' \
            ...
            --mlp_out_dim 256 \
            --use_dinov2 'True' \
            --checkpoint_path '/ceph/home/student.aau.dk/mdalal20/P10-project/hyperbolic-generalized-category-discovery/osr_novel_categories/metric_learn_gcd/log/CIFAR100-Hyperbolic-Angle-SGD-HCD-DINOv2-Train/checkpoints/model.pt'
            #--use_adam 'True' \
else
    srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            --dataset_name 'cifar100' \

...
#SBATCH --output="logs/GCD-Cifar100.log"
#SBATCH --job-name="GCD-Cifar100"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.contrastive_training.contrastive_training \
            ...
            --lr 0.1 \
            --eval_funcs 'v1' 'v2' \
            --exp_id 'Cifar100-Normal-Train' \
            --hyperbolic 'False' \
            --kmeans 'True' \
            --kmeans_frequency 20 \
            --checkpoint_path '/ceph/home/student.aau.dk/mdalal20/P10-project/hyperbolic-generalized-category-discovery/osr_novel_categories/metric_learn_gcd/log/Cifar100-Normal-Train/checkpoints/model.pt'

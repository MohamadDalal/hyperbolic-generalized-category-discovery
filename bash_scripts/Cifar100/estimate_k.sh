...
#SBATCH --output="logs/GCD-Estimate-Cifar100.log"
#SBATCH --job-name="GCD-Estimate-Cifar100"
...
outfile="logs/GCD-Estimate-Cifar100.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.estimate_k.estimate_k --max_classes 1000 --dataset_name cifar100 --search_mode other --warmup_model_exp_id 'Euclidean_best' --hyperbolic 'False'

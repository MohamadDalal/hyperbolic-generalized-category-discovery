...
#SBATCH --output="logs/GCD-Test-Cifar100.log"
#SBATCH --job-name="GCD-Test-Cifar100"
...
outfile="logs/GCD-Test-Cifar100.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.test_outputs.test_outputs --dataset_name cifar100 --warmup_model_exp_id 'Euclidean_best'

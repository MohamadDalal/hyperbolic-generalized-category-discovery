...
#SBATCH --output="logs/GCD-Extract-Cifar100.log"
#SBATCH --job-name="GCD-Extract-Cifar100"
...
outfile="logs/GCD-Extract-Cifar100.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.clustering.extract_features --dataset cifar100 --use_best_model 'True' \
 --warmup_model_dir '/ceph/home/student.aau.dk/mdalal20/P10-project/hyperbolic-generalized-category-discovery/osr_novel_categories/metric_learn_gcd/log/Cifar100-Normal-Train/checkpoints/model.pt' \

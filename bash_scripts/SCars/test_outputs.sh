...
#SBATCH --output="logs/GCD-Test-SCars.log"
#SBATCH --job-name="GCD-Test-SCars"
...
outfile="logs/GCD-Test-SCars.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.test_outputs.test_outputs --dataset_name scars --warmup_model_exp_id 'Euclidean_best' --eval_funcs "v2"

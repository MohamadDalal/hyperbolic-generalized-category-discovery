...
#SBATCH --output="logs/GCD-Estimate-SCars.log"
#SBATCH --job-name="GCD-Estimate-SCars"
...
outfile="logs/GCD-Estimate-SCars.out"
...
srun --output="${outfile}" --error="${outfile}" singularity exec --nv ${container_path} ${PYTHON} -m methods.estimate_k.estimate_k --max_classes 1000 --dataset_name scars --search_mode other --warmup_model_exp_id 'Euclidean_best' --hyperbolic 'False'
